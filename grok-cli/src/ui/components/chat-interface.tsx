...
import InterruptionDialog from "./interruption-dialog.js";
import {
  InterruptionService,
  InterruptionOptions,
} from "../../utils/interruption-service.js";
...
function ChatInterfaceWithAgent({
  ...
  const [interruptionOptions, setInterruptionOptions] =
    useState<InterruptionOptions | null>(null);
  ...
  useEffect(() => {
    ...
    const logoOutput = cfonts.render("GROKINATOR", {
      font: "3d",
      align: "left",
      colors: ["yellow", "blue"],
      space: true,
  ...
  useEffect(() => {
    const handleInterruptionRequest = (options: InterruptionOptions) => {
      setInterruptionOptions(options);
    };

    interruptionService.on("interruption-requested", handleInterruptionRequest);

    return () => {
      interruptionService.off(
        "interruption-requested",
        handleInterruptionRequest
      );
    };
  }, [interruptionService]);
  ...
  const handleInterruptionContinue = (skipFuturePrompts?: boolean) => {
    interruptionService.respondToInterruption(true, skipFuturePrompts);
    setInterruptionOptions(null);
  };

  const handleInterruptionStop = () => {
    interruptionService.respondToInterruption(false);
    setInterruptionOptions(null);

    // Reset processing states when interrupted
    setIsProcessing(false);
    setIsStreaming(false);
    setTokenCount(0);
    setProcessingTime(0);
  ...
  return (
    <Box flexDirection="column" paddingX={2}>
      ...
      {/* Show interruption dialog if one is pending */}
      {interruptionOptions && (
        <InterruptionDialog
          score={interruptionOptions.score}
          reason={interruptionOptions.reason}
          relevantMessages={interruptionOptions.relevantMessages}
          onContinue={handleInterruptionContinue}
          onStop={handleInterruptionStop}
        />
      )}

      {!confirmationOptions && !interruptionOptions && (
        <>
          <LoadingSpinner
